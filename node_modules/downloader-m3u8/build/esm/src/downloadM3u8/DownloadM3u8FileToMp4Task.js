import { getDownloadConfig, saveDownloadConfig, parseUrl, downloadM3u8KeyFileAndParseKey, connectTsFile, tranformMp4, deleteTmpPath, deleteTmpFile, } from './downloadM3u8FileToMp4';
...
export class DownloadM3u8FileMp4 {
    ...
    async startDownload() {
        ...
        if (m3u8Option.type == 0) {
            ...
            if (m3u8Option.realM3u8Url) {
                this.downloadOption.m3u8Url = parseUrl(m3u8Url, m3u8Option.realM3u8Url);
                return this.startDownload();
            }
            throw 'm3u8文件有问题，请查看';
        }
        m3u8Option = await downloadM3u8KeyFileAndParseKey(this.downloadOption, m3u8Option);
        if (this.state !== DownloadState.START)
            return;
        if (!downloadConfig.connectTsFile) {
            ...
            // console.log(JSON.stringify(m3u8Option));
            // 合并ts片段
            const depathTs = await connectTsFile(this.downloadOption, m3u8Option);
    ...
    async downloadM3u8TsFile(m3u8Option) {
        const { m3u8Url, filePath, title, headers, threadCount = 20, } = this.downloadOption;
        const { tsUrls } = m3u8Option;
        const downloadConfig = getDownloadConfig(this.downloadOption);
        const { hasDownloadFiles = [] } = downloadConfig;
        const pool = (this.pool = new TaskPool(threadCount, hasDownloadFiles.length));
        const tsFiles = [];
