...
import { parseM3u8File } from './m3u8FileParser';
...
export const getDownloadConfig = ({ filePath, title, }) => {
...
export const saveDownloadConfig = ({ filePath, title }, config) => {
    const configFile = path.resolve(filePath, title, `download.config`);
    saveJson(configFile, config);
};
export const deleteDownloadConfig = ({ filePath, title, }) => {
    const configFile = path.resolve(filePath, title, 'download.config');
    fs.removeSync(configFile);
};
export const downloadM3u8FileToMp4 = async (opts) => {
...
export const deleteTmpFile = (opts) => {
    const { filePath, title } = opts;
    const file = path.resolve(filePath, title, 'tmp');
    fs.removeSync(file);
};
export const deleteTmpPath = (opts) => {
    const { filePath, title } = opts;
    const file = path.resolve(filePath, title);
    fs.removeSync(file);
};
export const connectTsFile = async (opts, m3u8Option) => {
...
const downloadM3u8TsFile = async (opts, m3u8Option) => {
    const { m3u8Url, filePath, title, headers, threadCount = 20, onProgress, } = opts;
    ...
    tsUrls &&
        tsUrls.forEach((tsUrl, index) => {
            ...
            tsFiles.push(file);
            if (!fileSet.has(file)) {
                pool.addTask(new DownloadM3u8TsFileTask(tsUrl, file, headers || {}));
            }
        });
...
export const downloadM3u8KeyFileAndParseKey = async ({ m3u8Url, filePath, title, headers }, m3u8Option) => {
...
//相对路径换取正式网络地址
export const parseUrl = (sourceUrl, relativeUrl) => {
